...
class Template extends TemplateBase {
    ...
    public final function bindTo($tag, $value = null, $section = false, $autoFetch = false, $looseIt = true) {
		if( $tag === '' || (!empty($value) && $section == true && $autoFetch == true)) {
			return false;
		}
    ...
    final public function bindFileTo($tag, $path, $looseIt = true) {
	    if(!is_string($tag) || !is_string($path))
	       return false;
    ...
    public final function bindSelectFieldTo($tag, $name, $options = [], $params = null, $looseIt = true) {
		if(!is_string($tag) || $tag === '' || !is_string($name) || $name === '' || empty($options)){
			return false;
		}
	...
	{
		if(!is_string($tag) || $tag === '' || !is_string($name) || $name === '' || !is_string($type) || $type === ''){
			return false;
		}
		...
		switch ($type):
			...
			case 'image':
				if(!empty($img)){
					$html_string = '<input type="' . $type . '" name="' . $name . '" src="' . $img . '" value="' . $value . '" ' . $params . '/>';
				}
				else{
					$html_string = '<input type="' . $type . '" name="' . $name . '" value="' . $value . '" ' . $params . '/>';
				}
	...
	{
		if(!file_exists($path) || !is_string($sectionTag) || $sectionTag === '' || !is_string($sectionTag)) {
            return false;
        }
		...
		if(is_array($valueArray)){
				...
				while(list($section, $value) = each($values)) {
                    if(!is_array($value))
                        ...
                        while(list($key, $val) = each($value)) {
                            $singleRow = str_replace($key, $val, $singleRow);
                        }
                    }
				}
            ...
            $_internalIndexCount = array_key_exists($sectionTag, $this->_internalIndex) ? $this->_internalIndex[$sectionTag] : $this->_internalCount;
	...
	public final function setConfig($path, $section = null) {
        if(!is_string($path) || (isset($section) && !is_string($section)))
            return false;
        ...
        if(file_exists($path)) {
            ...
            while(list($key,$value)=each($conf_array)) {
                ...
                case 'parse':
                    if($value === true)
                        $this->setParseFlag(true);
                    else
                        $this->setParseFlag(false);
                    break;
                endswitch;
            }
	...
	final public function setParseConfig($parseValues) {
        ...
        while(list($k, $v) = each($parseValues)) {
    ...
    public final function getSection($path, $tag, $parse = false, $autoFetch = false)
	{
		...
		if(!file_exists($path) || !is_string($tag) || $tag === '') {
		...
		if($autoFetch)
		{
		  $_internalBuffer = $this->_fetch($_internalBuffer,$parse);
		}
		// return
		return $_internalBuffer;
	}
	...
	public final function checkSiteCache() {
        $found = '';
        if( ($found = $this->checkCache()) && is_string($found)) {
            self::display(parent::$_cachePath . $found, null, null, true);
        }
	}
	...
	public final function display($path, $docType = null, $parse = false, $cached = false) {
	    ...
	    if(!$cached) {
            ...
            if(!empty($docType))
                self::setDocType(intval($docType));
